...
const apiKey1 = `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`;
export default function ChatGPT() {
  ...
  useEffect(() => {
    generateCompletion(arrayUser[arrayUser.length - 1])
      .then((data) => {
  ...
  return (
    <div className={clsx(styles.chatGpt)}>
      <button onClick={handleHidden} className={clsx(styles.btnToggle)}>
            ...
            <div key={i}>
              ...
              {arrayUser.length > 0 && i <= arrayUser.length - 1 && (
                <div className={styles.user}>
                  <div className={clsx(styles.chatResponse)}>
                    <span className={clsx(styles.message)}>{arrayUser[i]}</span>

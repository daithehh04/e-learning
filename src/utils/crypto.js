...
export const enCryptClientPass = (_clientPass) => {
  return CryptoJS.AES.encrypt(_clientPass, SECRET_KEY_JOIN_CLASS).toString();
};

export const decryptClientPass = (_clientPass) => {
  const bytes = CryptoJS.AES.decrypt(_clientPass, SECRET_KEY_JOIN_CLASS);
  return bytes.toString(CryptoJS.enc.Utf8);
};

export function encodeSHA256Pass(userName, password) {
  try {
    const encode = CryptoJS.SHA256(
...
export function encodeSHA256Code(obj, key) {
  try {
